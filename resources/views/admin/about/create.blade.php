    ...
    <form class="space-y-4 w-1/2" action="{{route('about.store')}}" method="post" enctype="multipart/form-data">
        ...
        @for($i=1; $i<3; $i++)
            @foreach(config('app.locales') as $locale)
                <div>
                    <label for="title_{{ $i }}_{{ $locale }}" class="block capitalize mb-2 text-sm font-medium text-gray-900 dark:text-white">
                       başlık {{ $i }} {{ $locale }}
                    </label>
                    <input value="{{ $about?->getTranslation('title_'.$i, $locale) }}" type="text" name="title_{{ $i }}[{{ $locale }}]" id="title_{{ $i }}_{{ $locale }}"
                ...
                <div>
                    <label for="description_{{ $i }}_{{ $locale }}" class="block capitalize mb-2 text-sm font-medium text-gray-900 dark:text-white">açıklama {{ $i }} {{ $locale }}</label>
        ...
        @foreach(config('app.locales') as $locale)
            <div>
                <label for="quote_{{ $locale }}" class="block capitalize mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    alıntı {{ $locale }}
                </label>
                <input value="{{ $about?->getTranslation('quote', $locale) }}" type="text" name="quote[{{ $locale }}]" id="quote_{{ $locale }}"
            ...
            <div>
                ...
                @if ($about?->$field)
                    <div class="mb-2">
                        ...
                        <p class="text-sm text-gray-600">Mevcut resim. Yeni bir resim yüklemek için aşağıyı kullanın.</p>
                    </div>
                @endif
