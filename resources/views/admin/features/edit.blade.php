<x-app-layout>
    ...
    <div class="py-12">
        ...
        <div class="m-5" x-data="{
            init(){
                if(`{{ $feature->category->parent->parent_id }}`){
                    this.loadChildren(`{{ $feature->category->parent->parent_id }}`,1); // arsa
                    this.loadChildren(`{{ $feature->category->parent_id }}`,2); //kiralik
                }
                if(!`{{ $feature->category->parent->parent_id }}`){
                    this.loadChildren(`{{ $feature->category->parent_id }}`,1); //kiralik
                    this.loadChildren(`{{ $feature->category_id }}`,2); // arsa
                }
                this.selected2ndCategory = `{{ $feature->category_id }}`

        },
        firstLevelChildren:[],
        secondLevelChildren:[],
        filterable:0,
        selectedCategory:`{{ $feature->category_id }}`,
        selected1stCategory:null,
        selected2ndCategory:`{{ $feature->category_id }}`,
...
">
            ...
            <div class="p-4">
                ...
                <form class="space-y-4 flex flex-wrap justify-center" action="{{route('feature.update',$feature)}}"
                    method="post">
                    @csrf
                    ...
                    <div class="w-full ">
                        <div class="w-1/3 mx-auto">
                            ...
                            <input type="text" name="name[{{ $locale }}]" id="infos"
                                ...
                                required value="{{ $feature->getTranslation('name',$locale) }}" />
                        </div>
                    </div>
                    @endforeach
