<x-app-layout>
    ...
    sectionFeatures:1
        ...
        <form class="space-y-4 w-1/2" action="{{route('service.update',$service)}}" method="post" enctype="multipart/form-data">
            <div class="mt">
                @if($errors->all())
            ...
            <div>
                ...
                <input type="text" value="{{ $service->getTranslation('name',$locale) }}" name="name[{{ $locale }}]" id="team"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                    required />
            </div>
            @endforeach
            <div>
                <label for="team" class="block capitalize mb-2 text-sm font-medium text-gray-900 dark:text-white">Video
                    Linki</label>
            ...
            <div>
                <div class="mb-2">
                    ...
                    <select id="department" name="department_id"
                        ...
                        <option @selected($department->id == $service->department_id ) value="{{ $department->id }}">{{$department->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
@foreach(config('app.locales') as $locale)
    <input placeholder="{{ $locale }}" value="{{ $feature->getTranslation('value',$locale) }}" type="text" name="features[{{ $index }}][{{ $locale }}]"
    ...
    @endforeach
            ...
            @foreach($service->subServices as $index=>$subService)
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $subService->getTranslation('name',$locale) }}" name="sub_services[{{ $index }}][name][{{ $locale }}]" id="team"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                required />
                        </div>
                        @endforeach
                        ...
                        <div class="flex justify-evenly">
                        @foreach(config('app.locales') as $locale)
                            <input value="{{ $feature->getTranslation('value',$locale) }}" placeholder="{{ $locale }}" type="text" name="sub_services[{{ $index }}][features][{{ $findex }}][{{ $locale }}]"
                            class="bg-gray-50 mx-3 my-2 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="₺2999">
                            @endforeach
                        </div>
...
<template x-for="feature,index in sectionFeatures">
    <div class="flex justify-evenly">
        <input placeholder="tr" type="text" :name="`sub_services[{{ $index }}][features][${index+700}][tr]`"
            class="bg-gray-50 mx-3 my-2 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">

        <input placeholder="ru" type="text" :name="`sub_services[{{ $index }}][features][${index+700}][ru]`"
            class="bg-gray-50 border mx-3 my-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">

        <input placeholder="en" type="text" :name="`sub_services[{{ $index }}][features][${index+700}][en]`"
            class="bg-gray-50 border mx-3 my-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
    </div>
</template>
