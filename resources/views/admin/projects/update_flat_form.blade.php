<section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        <h2 class="mb-4 text-xl font-bold text-indigo-600 text-center dark:text-white capitalize">{{ __('yeni daire') }}
        </h2>
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="w-full">
                <label for="rooms"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('oda sayısı') }}</label>
                ...
                <div class="sm:col-span-2">
                    @if ($loop->first)
                        <label for="flat_description"
                            class="block mb-2 text-2xl font-semibold capitalize text-gray-900 dark:text-white">{{ __('general.description') }}</label>
                    @endif
                    <textarea :name="`flats[${flatIndex+500}][description][{{ $locale }}]`" rows="4"
