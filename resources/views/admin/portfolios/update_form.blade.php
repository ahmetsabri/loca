            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title[tr]" id="title_tr"
                        ...
                        value="{{ old('title[tr]') ?? $portfolio->getTranslation('title', 'tr') }}">
                    <br>
                    <label for="title_tr" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        başlık rusça
                    </label>
                    ...
                    <input type="text" name="title[ru]" id="title_ru"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="title in" required
                        value="{{ old('title[ru]') ?? $portfolio->getTranslation('title', 'ru') }}">
                    <br>
                ...
                <div>
                    ...
                    <select id="user" name="user_id"
                        ...
                        @foreach ($users as $user)
                            <option @selected($user->id == $portfolio->user_id) value="{{ $user->id }}">{{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="province" name="province_id"
                        ...
                        @foreach ($provinces as $province)
                            <option @selected($province->id == $portfolio->province_id) value="{{ $province->id }}">{{ $province->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select id="district" name="district_id"
                        ...
                        <template x-for="district in districts">
                            <option x-text="district.name" :value="district.id"
                                :selected="district.id == portfolioDistrict"></option>
                        </template>
                    </select>
                </div>
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="promotion_url">
                        tanıtım linki
                    </label>
                    <input
                        ...
                        id="promotion_url" type="url" name="promotion_url" placeholder="tanıtım linki"
                        value="{{ old('promotion_url') ?? $portfolio->promotion_url }}">
