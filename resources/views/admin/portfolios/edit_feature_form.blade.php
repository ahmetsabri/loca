    ...
    <li class="w-1/3 border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
        <div class="flex items-center ps-3">
            <input id="vue-checkbox-{{ $option->id }}" type="checkbox" value="{{ $option->id }}"
                ...
                @checked(in_array($option->id,$portfolio->features->pluck('feature_option_id')->toArray())) >
            <label for="vue-checkbox-{{ $option->id }}"
                class="w-full py-3 ms-2 text-sm font-semibold text-indigo-600 cursor-pointer dark:text-gray-300">{{
                $option->name }}</label>
        </div>
    </li>
    @endforeach
