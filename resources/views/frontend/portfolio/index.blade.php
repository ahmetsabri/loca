...
@section('title', 'Portföyler | W Property')
                        ...
                        <form method="GET" action="{{ route('portfolios') }}"
                            ...
                            <div class="sidebar-general-list bg-white rounded-5 md:rounded-3 px-4 py-7">
                                <div class="category-wrapper px-3">
                                    ...
                                    @else
                                    @if (in_array(request('filter.category'),
                                            $category->children->pluck('id')->merge($category->id)->toArray()))
                                            active @endif
                                    @endif
                                    ">
                                            @if ($category->children->isNotEmpty())
                                                <button type="button" data-destination-level="s{{ $category->id }}"
                                                    class="text-tertiary-950 @if (in_array(request('filter.category'),
                                                            $category->children->pluck('id')->merge($category->id)->toArray())) active block @endif text-3.5 w-fit font-medium draw-underline [--line-color:#0D1523]  capitalize">
                                                    {{ $category->name }}
                                                </button>
                                            @else
                                    ...
                                    @foreach ($rootCategories as $category)
                                        <div data-level="s{{ $category->id }}"
                                            class="list hidden   @if (in_array(request('filter.category'),
                                                    $category->children->pluck('id')->merge($category->id)->toArray())) active @endif [&.active]:grid gap-1 overflow-y-auto overflow-x-hidden max-h-[153px] scrollbar scrollbar-w-1 scrollbar-track-rounded-full scrollbar-thumb-rounded-full scrollbar-track-[#8AA5D3]/24 scrollbar-thumb-[#8AA5D3]/65">
